...
- **Completion checklist**: every `SKILL.md` ends with a `## Completion Checklist` — a blocking self-verification gate the agent must satisfy (or have the user explicitly waive) before reporting the skill done. Per the completion-checklist convention in `astrodb-directions.md`, it is **file-tracked**: at the start the skill copies its items, unchecked, to `<artifact-dir>/<skill-name>-checklist.md`, flips each `[ ]` to `[x]` with a one-line evidence note as the item completes, reads the file back before finishing, and reproduces the evidence-annotated checklist in its completion message. Items are phrased as verifiable outcomes, and user-dependent ones as "prompted and honored the user's choice (or they declined)," never as a forced action that could tempt fabrication. The file-tracking protocol itself lives once in `astrodb-directions.md`; each `SKILL.md` keeps only its own checklist items plus a short pointer to that convention. When you add or restructure a skill's steps, update its checklist items in the same edit.
