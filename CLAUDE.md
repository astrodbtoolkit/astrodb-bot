...
- **Per-workflow artifact directories**: each workflow writes into its own directory in the project root — **`astrodb-build-artifacts/`** (parsed tables, generated schema YAML), **`astrodb-ingest-artifacts/`** (generated ingestion scripts), and **`astrodb-website-artifacts/`**. Each also holds that phase's own decision log — **`build-workflow.md`** / **`ingest-workflow.md`** / **`website-workflow.md`** — there is no shared project-root `workflow.md`. Only the build workflow additionally persists a `checklists.md` there (see Completion checklist below).
