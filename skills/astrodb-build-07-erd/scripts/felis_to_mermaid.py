...
def main():
    ...
    if args.split:
        ...
        for cluster in ("lookup", "main", "data"):
            ...
            if not names:
                continue
            # Boundary tables appear as keys-only context boxes so edges have somewhere to land.
            context = set()
            for rel in rels:
            ...
            if args.stats:
                report_stats(
                    tables, rels, names, sorted(context), diagram, args.max_chars, cluster
                )
    ...
    if args.cluster:
        ...
        context = set()
        for rel in rels:
            if rel["child"] in include and rel["parent"] not in include:
                context.add(rel["parent"])
            if rel["parent"] in include and rel["child"] not in include:
                context.add(rel["child"])
        context -= exclude
    elif args.tables:
        ...
        if missing:
            print(
                "ERROR: table(s) not in the schema: %s" % ", ".join(missing),
                file=sys.stderr,
            )
            sys.exit(1)
        context = set()
        ...
        include = [n for n in all_names if n not in exclude]
        context = set()
